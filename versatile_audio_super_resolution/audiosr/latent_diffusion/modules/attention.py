...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        # Use different attention backends based on global setting
        ...
        if backend == 'sdpa':
            ...
            if exists(mask):
                ...
                # Reshape mask for SDPA: [batch, heads, n, n]
            ...
            try:
                import flash_attn
                q_flash = rearrange(q, "(b h) n d -> b h n d", h=h)
                k_flash = rearrange(k, "(b h) n d -> b h n d", h=h)
                v_flash = rearrange(v, "(b h) n d -> b h n d", h=h)

                # Check dtype - Flash Attention only supports fp16/bf16
                supported_dtype = q_flash.dtype in (torch.float16, torch.bfloat16)

                # Flash Attention doesn't support mask in the same way, handle separately
                if not exists(mask) and supported_dtype:
                    out = flash_attn.flash_attn_func(q_flash, k_flash, v_flash, dropout_p=0.0)
                    out = rearrange(out, "b h n d -> b n (h d)", h=h)
                else:
                    # Fallback to SDPA for masked attention or unsupported dtype
                    if not supported_dtype and not exists(mask):
                        print("[Flash Attention] Falling back to SDPA (requires fp16/bf16, got {})".format(q_flash.dtype))
                    q_sdpa = rearrange(q, "(b h) n d -> b h n d", h=h)
                    k_sdpa = rearrange(k, "(b h) n d -> b h n d", h=h)
                    v_sdpa = rearrange(v, "(b h) n d -> b h n d", h=h)
                    if exists(mask):
                ...
                # Fallback to SDPA if flash_attn not available
                q_sdpa = rearrange(q, "(b h) n d -> b h n d", h=h)
                k_sdpa = rearrange(k, "(b h) n d -> b h n d", h=h)
                v_sdpa = rearrange(v, "(b h) n d -> b h n d", h=h)
                if exists(mask):
